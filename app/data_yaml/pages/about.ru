...
Jetware предоставляет менеджер рабочих сред и коллекцию пакетов для работы серверных приложений. Это могут быть как популярные веб-приложения такие как Wordpress, Drupal, Redmine, Confluence и другие, так и ваши собственные разработки. Онлайн-сервис и программы предоставляются бесплатно.
#### Рабочее окружение – аплайнс

Рабочее окружение веб-приложения поставляется в виде аплайнса. Аплайнс - это полный комплект всех программ и библиотек, используемых этим приложением. Для каждого приложения можно создать аплайнс с использованием различных программ, их версий, настроек и т. д.
Все программы из коллекции Jetware оптимизированы для веб-задач и настроены на взаимодействие друг с другом. Аплайнс готов к использованию сразу же после создания, и не требует установки дополнительных
...
Созданный аплайнс можно использовать повторно сколько угодно раз, а также делиться им с другими.
...
Мы отслеживаем и выпускаем обновления для всех программ из нашей коллекции. Вам не нужно заниматься ручной сборкой "configure make install" и каждый год, при необходимости установить новую версию, вспоминать как вы собирали этот пакет раньше. Вы получаете готовые обновления каждого используемого пакета.
